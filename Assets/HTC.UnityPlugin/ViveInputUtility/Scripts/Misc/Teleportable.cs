...
namespace HTC.UnityPlugin.Vive
{
    ...
    public class Teleportable : MonoBehaviour, ReticlePoser.IMaterialChanger, IPointer3DPressExitHandler
    {
        ...
        public void OnPointer3DPressExit(Pointer3DEventData eventData)
        {
            ...
            if (eventData.TryGetViveButtonEventData(out viveEventData))
            {
                switch (teleportButton)
                {
                    ...
                    case TeleportButton.Menu: if (viveEventData.viveButton != ControllerButton.Menu) { return; } break;
                }
            }
            else if (eventData.button != (PointerEventData.InputButton)teleportButton)
            {
                switch (teleportButton)
                {
                    ...
                    case TeleportButton.Menu: if (eventData.button != PointerEventData.InputButton.Middle) { return; } break;
                }
            }
            //if (eventData.TryGetViveButtonEventData(out viveEventData))
            //{
            //    if(viveEventData.viveButton != ControllerButton.Menu)
            //    {
            //        return;
            //    }
            //}
